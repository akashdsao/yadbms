...
public class LRUKReplacer {
    ...
    /**
     * Attempts to select a frame to be replaced.
     * Implements LRUk (Least Recently Used K) replacement policy. In case of a tie, it selects the oldest frame.
     * If no frame is available, it returns an empty Optional.
     *
    ...
    public synchronized Optional<FrameId> victim() {
        ...
        for (Map.Entry<FrameId, LRUKNode> entry : nodes.entrySet()) {
            ...
            if (node.isEvictable() &&
                    (victimNode.isPresent() || distance > maxDistance || (distance == maxDistance && old < oldestTimestamp))) {
     ...
     * This method should only be called for frames that are evictable.
     *
    ...
    public synchronized void removeFrame(FrameId frameId) {
        ...
        if (!nodes.get(frameId).isEvictable()) {
            throw new DBException(ErrorType.INVALID_OPERATION, "Cannot remove a pinned frame: " + frameId);
        }
        nodes.remove(frameId);
    }

    /**
     * Records access to a frame, updating its usage status.
     * This method is typically called when a frame is accessed, to keep track of its usage.
     *

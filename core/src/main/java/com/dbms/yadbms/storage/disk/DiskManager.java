...
/**
 * DiskManager is responsible for managing the database file and log file on disk.
...
public class DiskManager {
    ...
    /**
     * Constructs a DiskManager with the specified database file path.
     * Initializes the log file and database file channels, and sets up a shutdown hook to close them on exit.
     *
    ...
    /**
     * Writes a page to the database file at the specified PageId.
     * If the page does not exist, it allocates a new page.
     *
    ...
    public synchronized void writePage(PageId pageId, byte[] pageData) {
        long offset = pages.computeIfAbsent(pageId, id -> {
            try {
                return allocatePage();
            } catch (IOException e) {
                throw new DBException(ErrorType.IO_ERROR, "Failed to allocate page for pageId: " + pageId.toString());
            }
        });
    ...
    /**
     * Reads a page from the database file at the specified PageId.
     * If the page does not exist, it allocates a new page, lazily initializing it for logical consistency.
     *
    ...
    /**
     * Deletes a page from the database file at the specified PageId.
     * The page is marked as deleted by adding its offset to the free slots.
     *
     ...
     * The log entry is appended to the end of the log file and flushed to disk.
     *
    ...
    /**
     * Reads a log entry from the log file at the specified offset.
     * The log entry is read into the provided byte array.
     *
     ...
     * If there are free slots available, it reuses one of them.
     * If the current page capacity is exceeded, it doubles the capacity and truncates the file accordingly.
     *
